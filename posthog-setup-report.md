...
- [x] Add `PUBLIC_POSTHOG_PROJECT_TOKEN` and `PUBLIC_POSTHOG_HOST` to any CI/CD environment variable configuration (GitHub Actions secrets or deployment platform env vars) so the build has access to them.
- [x] Wire source-map upload (`posthog-cli sourcemap` or your bundler's upload step) into CI so production stack traces de-minify in PostHog error tracking.
